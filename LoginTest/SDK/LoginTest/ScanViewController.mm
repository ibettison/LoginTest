...
{
	ScanViewController* p = (ScanViewController*)pThis;
	[p onError:str];
...
-(void)PlaySound:(NSString *)resourceName withFileExtension: (NSString *)extName {
    ...
    [audioPlayer play]; //will need to release this memory after showing the erver information
...
{
    [self PlaySound:@"store-scanner-beep" withFileExtension:@"mp3"];
    NSString *strLocal;
	strLocal = [NSString stringWithFormat:@"%S" , str];
	[textContainer setText:strLocal];
